    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if is_valid_location(board, col):
                ...
                if not game_over:
                    # AI's turn
                    col, _ = minimax(board, 2, True)  # Adjust the depth for AI difficulty
                    if is_valid_location(board, col):
                        pygame.time.wait(500)  # Pause for a moment to make AI moves visible
                        row = get_next_open_row(board, col)
                        drop_piece(board, row, col, 2)  # AI's piece (2)

                        if winning_move(board, 2):  # Check if AI wins
